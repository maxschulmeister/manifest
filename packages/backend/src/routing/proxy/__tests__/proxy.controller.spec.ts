...
describe('ProxyController', () => {
  ...
  it('stores a synthetic turn key for custom-tier alias tool requests without trace headers', async () => {
    routingAliasService.listConfiguredAliases.mockResolvedValue(['auto', 'fast']);
    headerTierService.findByModelAlias.mockResolvedValue({ id: 'ht-fast', name: 'fast' });
    proxyService.proxyRequest.mockResolvedValue({
      forward: {
    ...
    expect(mockMessageRepo.insert).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        session_id: expect.stringMatching(/^turn:/),
      }),
    );
  });

  it('stores a synthetic turn key for traced custom-tier alias tool requests', async () => {
    routingAliasService.listConfiguredAliases.mockResolvedValue(['auto', 'fast']);
    headerTierService.findByModelAlias.mockResolvedValue({ id: 'ht-fast', name: 'fast' });
    proxyService.proxyRequest.mockResolvedValue({
      forward: {
  ...
  it('stores a synthetic turn key for header-selected custom tiers without trace headers', async () => {
    proxyService.proxyRequest.mockResolvedValue({
      forward: {
        response: new Response(
          JSON.stringify({
    ...
    req.headers = { 'x-session-key': 'my-session', 'x-manifest-tier': 'fast' };
    const { res } = mockResponse();
  ...
  describe('streaming', () => {
    ...
    it('stores a synthetic turn key for streaming custom-tier responses', async () => {
      routingAliasService.listConfiguredAliases.mockResolvedValue(['auto', 'fast']);
      headerTierService.findByModelAlias.mockResolvedValue({ id: 'ht-fast', name: 'fast' });
      const mockProviderResp = createMockStreamResponse([
        'data: {"choices":[{"delta":{"content":"hi"}}]}\n\n',
