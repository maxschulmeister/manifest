...
export class ProviderParamSpecService implements OnModuleInit {
  ...
  async getSpecs(
    ...
    const directSpecs = this.getCatalogSpecs(providerId, authType, model);
    ...
    return this.getCatalogSpecs(metadata.provider, authType, metadata.model).map((spec) =>
      withRouteIdentity(spec, providerId, authType, model),
    );
  }
