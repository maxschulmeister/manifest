/**
 * Adapted from pi-cursor-sdk cursor-mcp-timeout-override.ts (MIT). See ATTRIBUTION.md.
 */
...
const CURSOR_MCP_TOOL_TIMEOUT_MS_ENV = 'MANIFEST_CURSOR_MCP_TOOL_TIMEOUT_MS';
const CURSOR_MCP_TOOL_TIMEOUT_SECONDS_ENV = 'MANIFEST_CURSOR_MCP_TOOL_TIMEOUT_SECONDS';
const CURSOR_MCP_CONNECT_TIMEOUT_MS_ENV = 'MANIFEST_CURSOR_MCP_CONNECT_TIMEOUT_MS';
const CURSOR_MCP_CONNECT_TIMEOUT_SECONDS_ENV = 'MANIFEST_CURSOR_MCP_CONNECT_TIMEOUT_SECONDS';
...
export function resolveMcpPatchedTimeoutDelay(
  delay: SetTimeoutDelay,
  stack: string | undefined,
  toolTimeoutMs: number = installedToolTimeoutMs,
  connectTimeoutMs: number = installedConnectTimeoutMs,
): SetTimeoutDelay {
  if (!isCursorSdkDefaultMcpTimeout(delay)) return delay;
  if (isCursorSdkMcpToolTimeoutStack(stack)) return toolTimeoutMs;
  if (isCursorSdkMcpConnectTimeoutStack(stack)) return connectTimeoutMs;
  return delay;
}
...
function patchedSetTimeout(
  ...
  const nextDelay = resolveMcpPatchedTimeoutDelay(delay, new Error().stack);
...
export const __cursorMcpTimeoutTestUtils = {
  patchedSetTimeoutForTests: patchedSetTimeout,
  isCursorSdkDefaultMcpTimeoutForTests: isCursorSdkDefaultMcpTimeout,
};
